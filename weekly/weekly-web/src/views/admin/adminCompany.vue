...
//        console.log(type);
        ...
        // else if(!this.formUser.usernum){ this.$message.warning('请输入负责人id');}
          ...
          this.formUser.orgId=this.orgId;
          this.formUser.orgNodeId=this.orgNodeId;
          this.addUpdateCompany({formUser:this.formUser,tenantId:tenantId,userInfo:this.userInfo}).then(res => {
            if (res.errno == 0) {
      ...
      confirmDelete(){
        ...
        this.deleteCompany({orgId:this.selectedItem.orgId,id:this.selectedItem.id,userInfo:this.userInfo}).then( res => {
          if(res.errno == 0){
